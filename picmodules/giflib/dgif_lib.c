...
#define READ(_gif,_buf,_len)                                     \
  (((GifFilePrivateType*)_gif->Private)->Read ?                   \
	...
	IMAGE_FILE_READ(_buf,1,_len,((GifFilePrivateType*)_gif->Private)->filehandler))
...
GifFileType *DGifOpenFileName(const char *FileName)
{
 ...
 //   FILE *f;
 	IMAGE_FILE_HANDLER hdl;
	int a= sizeof(GifFilePrivateType);
    GifFile = (GifFileType *)IMAGE_MALLOC(sizeof(GifFileType));
    if (GifFile == NULL) {
    ...
    Private = (GifFilePrivateType *)IMAGE_MALLOC(sizeof(GifFilePrivateType));
    if (Private == NULL) {
        _GifError = D_GIF_ERR_NOT_ENOUGH_MEM;
 
        IMAGE_FREE((char *)GifFile);
        return NULL;
    }
#if defined(__MSDOS__) || defined(WINDOWS32) || defined(_OPEN_BINARY)
...
//    f = fopen(FileName, "rb");    /* Make it into a stream: */
	hdl = IMAGE_FILE_OPEN(FileName,IMAGE_FILE_OPEN_READ_MODE);
...
//        fclose(f);
		IMAGE_FILE_CLOSE(hdl);
        IMAGE_FREE((char *)Private);
        IMAGE_FREE((char *)GifFile);
        return NULL;
    }
...
DGifGetImageDesc(GifFileType * GifFile) {
    ...
    if (GifFile->SavedImages) {
        if ((GifFile->SavedImages = (SavedImage *)IMAGE_REALLOC(GifFile->SavedImages,
                                      sizeof(SavedImage) *
        ...
        if ((GifFile->SavedImages =
             (SavedImage *) IMAGE_MALLOC(sizeof(SavedImage))) == NULL) {
            _GifError = D_GIF_ERR_NOT_ENOUGH_MEM;
            return GIF_ERROR;
        }
    }
    ...
    do {
        ...
        switch (RecordType) {
          case IMAGE_DESC_RECORD_TYPE:
              ...
              sp->RasterBits = (unsigned char *)IMAGE_MALLOC(ImageSize *
                                                       sizeof(GifPixelType));
