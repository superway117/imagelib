...
#define WRITE(_gif,_buf,_len)   \
  (((GifFilePrivateType*)_gif->Private)->Write ?    \
   ...
   IMAGE_FILE_WRITE(_buf, 1, _len, ((GifFilePrivateType*)_gif->Private)->filehandler))
...
GifFileType *EGifOpenFileName(const char *FileName,
 ...
 GifFileType *EGifOpenFileName(const char *FileName,int TestExistance) 
 {
   ...
   // f = fopen(FileName, "wb");    /* Make it into a stream: */
   hdl = IMAGE_FILE_OPEN(FileName,IMAGE_FILE_OPEN_CREATE_WRITE_MODE);
...
//    File = Private->File;
    ...
    if (Private) {
        if (Private->HashTable) {
            IMAGE_FREE((char *) Private->HashTable);
        }
