...
MakeMapObject(int ColorCount,
    ...
    Object = (ColorMapObject *)IMAGE_MALLOC(sizeof(ColorMapObject));
    if (Object == (ColorMapObject *) NULL) {
        return ((ColorMapObject *) NULL);
    }

    Object->Colors = (GifColorType *)IMAGE_CALLOC(ColorCount, sizeof(GifColorType));
...
UnionColorMap(const ColorMapObject * ColorIn1,
    ...
    if (RoundUpTo != ColorUnion->ColorCount) {
        ...
        if (RoundUpTo < ColorUnion->ColorCount)
            ColorUnion->Colors = (GifColorType *)IMAGE_REALLOC(Map,
...
AddExtensionBlock(SavedImage * New,
    ...
    if (New->ExtensionBlocks == NULL)
        New->ExtensionBlocks=(ExtensionBlock *)IMAGE_MALLOC(sizeof(ExtensionBlock));
    else
        New->ExtensionBlocks = (ExtensionBlock *)IMAGE_REALLOC(New->ExtensionBlocks,
    ...
    ep->ByteCount=Len;
    ep->Bytes = (char *)IMAGE_MALLOC(ep->ByteCount);
    if (ep->Bytes == NULL)
        return (GIF_ERROR);
...
FreeExtension(SavedImage * Image)
{
    ...
    for (ep = Image->ExtensionBlocks;ep < (Image->ExtensionBlocks + Image->ExtensionBlockCount); ep++)
        //(void)IMAGE_FREE((char *)ep->Bytes);
		IMAGE_FREE((char *)ep->Bytes);
    IMAGE_FREE((char *)Image->ExtensionBlocks);
...
MakeSavedImage(GifFileType * GifFile,
    ...
    if (GifFile->SavedImages == NULL)
        GifFile->SavedImages = (SavedImage *)IMAGE_MALLOC(sizeof(SavedImage));
    else
        GifFile->SavedImages = (SavedImage *)IMAGE_REALLOC(GifFile->SavedImages,
    ...
    if (GifFile->SavedImages == NULL)
        ...
        if (CopyFrom) {
            ...
            /* next, the raster */
            sp->RasterBits = (unsigned char *)IMAGE_MALLOC(sizeof(GifPixelType) *
            ...
            if (sp->ExtensionBlocks) {
                sp->ExtensionBlocks = (ExtensionBlock *)IMAGE_MALLOC(
                                      sizeof(ExtensionBlock) *
...
FreeSavedImages(GifFileType * GifFile) {
    ...
    IMAGE_FREE((char *)GifFile->SavedImages);
